...
E1=1; E2=1; E3=1; E4=1
make clean && make CFLAGS_EXTRA="$CF" -j$PM ENABLE_DEDUPE=1 static_jc static_stripped && cp $NAME$EXT $PKGNAME/$NAME$EXT && E1=0
make clean && make CFLAGS_EXTRA="$CF" -j$PM ENABLE_DEDUPE=1 LOUD=1 static_jc static_stripped && cp $NAME$EXT $PKGNAME/${NAME}-loud$EXT && E2=0
make clean && make CFLAGS_EXTRA="$CF" -j$PM LOW_MEMORY=1 static_jc static_stripped && cp $NAME$EXT $PKGNAME/${NAME}-lowmem$EXT && E3=0
make clean && make CFLAGS_EXTRA="$CF" -j$PM BARE_BONES=1 static_jc static_stripped && cp $NAME$EXT $PKGNAME/${NAME}-barebones$EXT && E4=0
